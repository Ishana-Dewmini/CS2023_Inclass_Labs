...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        int hash = 0;
        //add your code below
        for(int digit = 0; digit < user_name.length(); digit++){
            sum += (int) user_name[digit];
        }
        hash = sum % MAX_LENGTH;
        return hash;
    }
	...
	//add an if condition to complete the code here
        if(empty){
            ...
            cout<<"User adding is failed. Hash table is full.\n";
        }
	...
	//add an if condition to complete the code here
	    if(empty){
            ...
            cout<<"Password of user "<<user_name<<": "<<password[hash]<<endl;
        }
